...
body <- dashboardBody(
  tabItems(
    ...
    tabItem(tabName = "explore",
            h2("Explore the data"),
            fluidRow(
              column(width = 4, selectInput(inputId = "county", label = "Select a County:",
                                 choices = unique(CDPRCuApp$COUNTY_NAME), multiple = TRUE)),
              column(width = 4, selectInput(inputId = "crop", label = "Select agricultural applications:",
                                 choices = unique(CDPRCuApp$SITE_NAME), multiple = TRUE)),
              column(width = 4, selectInput(inputId = "chem", label = "Select active ingredients:",
                                 choices = unique(CDPRCuApp$CHEMICAL_NAME), multiple = TRUE))
            ),
            fluidRow(
  ...
  exploreData <- reactive({
    CDPRCuApp %>%
      filter(COUNTY_NAME %in% input$county,
             SITE_NAME %in% input$crop,
             CHEMICAL_NAME %in% input$chem)
  })
  
  output$kgAppBox <- renderInfoBox({
    infoBox(title = "Total kg of Copper-based pesticide applied", value = sum(exploreData()$KG_CHEMICAL_APPLIED))
  })
